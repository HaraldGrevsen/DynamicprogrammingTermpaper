...
ax.set_xlabel(f"age")
ax.set_ylabel(f"Human capital $k_t$")
ax.set_title(f'Experience')
plt.show()
...
p = model.sim.p
s = model.sim.s
av_h = np.mean(model.sim.h,1)

shock1 = model.par.eps_w
shock2 = model.par.eps_ts

prob = model.sim.prob
prob_p = model.sim.prob[1,:]
prob_u = model.prob[0,:]
sum = model.prob[2,:]+model.prob[1,:]+model.prob[0,:]
